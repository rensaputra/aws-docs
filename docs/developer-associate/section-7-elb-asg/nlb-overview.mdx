...
- **Protocol Focus**: The NLB operates exclusively at the transport layer, handling raw **TCP and UDP** traffic. It doesn't look at HTTP headers, cookies, or URL paths.
...
- **The Network Identity**: Unlike an ALB (which uses dynamic IP addresses that constantly change under its DNS record), an NLB allocates exactly **one static IP address per AZ**.
...
- **EC2 Instances**: Routing raw TCP/UDP streams directly to a fleet of compute servers/
...
**Question 1**: You are running a public DNS service on an EC2 instance where the DNS name is pointing to the IP address of the instance. You wish to upgrade your DNS service but would like to do it without any downtime.
...
- [ ] Provide a static private IP
  - **Explanation**: Private IP addresses are not publicly routable over the internet, so users outside your VPC would not be able to reach your public DNS service.
- [ ] Create a Load Balancer and an auto scaling group
  - **Explanation**: AWS Elastic Load Balancers expose a DNS domain name rather than a static public IP address. Since DNS clients query specific public IP addresses, an ALB does not fit this requirement.
- [ ] Use Route 53
  - **Explanation**: Amazon Route 53 is AWS's managed DNS service. However, the scenario specifically asks about upgrading a custom DNS software service running directly on an EC2 instance, not migrating to Route 53.
- [x] Elastic IP
  - **Explanation**: An Elastic IP (EIP) address is a static, publicly routable IPv4 address designed for dynamic cloud computing. You can associate an Elastic IP with a new EC2 instance running the upgraded DNS software, instantly remapping the public IP from the old instance to the new instance with zero downtime for clients querying that IP.
