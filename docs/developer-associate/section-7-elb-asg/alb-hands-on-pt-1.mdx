...
- **The Proof of Balancing**: When you hot the ALB's DNS name in a browser and repeateadly refresh the page, the underlying instance ID changes. This proves the ALB is actively round-robining traffic across your backend fleet.

### High-Performace Load Balancer Comparison

Three tiers of AWS load balancers:

- **ALB**: Built for **HTTP and HTTPS** traffic at Layer 7
- **NLB**: Engineered fot **TCP, UDP, and TLS** traffic at Layer 4. Choose this when you need **ultra-high performance, millions of requests per second**, and ultra-low latency.
...
- **The Security Group Lockdown**: It's not being shown in the lecture, but the ideal security group configuration for an ALB is to have the EC2 instance's security group only allow inbound traffic from the ALB's security group. This ensures that all traffic to the backend servers must pass through the ALB, preventing users from bypassing the load balancer and hitting the servers directly. This is a common exam question scenario, so make sure to understand this pattern.
