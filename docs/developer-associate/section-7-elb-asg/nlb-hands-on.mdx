...
- **The Advantage**: This proves that an NLB can perform application-level vetting. It won't just check if the port is open; it actively verifies that your Apache/web software layer is alive and responging with a `200 OK` status.
...
- **The Result**: The moment that rule is saved, the health checks flip to `healthy`, and the NLB's DNS name immediately begins round-robining traffic across the instances.
