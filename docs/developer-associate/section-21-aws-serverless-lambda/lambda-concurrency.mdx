...
**Question**: The development team at a retail company is gearing up for the upcoming Thanksgiving sale and wants to make sure that the application's serverless backend running via Lambda functions does not hit latency bottlenecks as a result of the traffic spike.
...
Configure Application Auto Scaling to manage Lambda provisioned concurrency on a schedule.
You can configure both reserved and provisioned concurrency for a Lambda function. However, **reserved concurrency is used to limit the maximum number of concurrent executions** for a Lambda function, while **provisioned concurrency is used to pre-warm a certain number of execution environments** to reduce cold start latency. In this case, since the goal is to address latency bottlenecks during a traffic spike, configuring provisioned concurrency on a schedule would be the appropriate solution. Please see note for more details:
![](./img/lambda-concurrency-practice-scenario.png)
