...
- **The Persistent Endpoint Daemon (`sam local start-lambda`)**:
  ...
  - _The Dev Win:_ Perfect if you want to point automated testing suites, integration runner scripts, or local testing tools directly at a simulated Lambda target environment without hitting the live web, chief!
...
- **The Mock Server Setup:** Spawns a localized HTTP mock web environment (running by default on `[http://127.0.0.1:3000](http://127.0.0.1:3000)`) that reads your `template.yaml` routing filters.
