...
- **The Launch Essentials (The Setup)**: First of all go to EC2 Dashboard and click on **Launch Instance**
  ...
  - **Key Pairs**: Essential for SSH access. It's like a password, but way more secure. You create a key pair, download the private key, and AWS keeps the public key. When you SSH into your instance, it checks if you have the right private key. Download the `.pem` file (Linux/Mac/Windows 10) or `.ppk` file (older Windows).
- **Networking & Security**:
...
- **User Data**:
  ...
  - **What it does**: It automatically updated the system, installed a webserver (`httpd`), started it, and created "Hello World" page. This is called **bootstrapping**.
