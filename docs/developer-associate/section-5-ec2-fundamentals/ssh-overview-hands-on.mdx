...
- **Convert the Key**: PuTTY doesn't understand `.pem` files, so you need to convert it to `.ppk` using PuTTYgen.
  - Open PuTTYgen, load your `.pem` file, and then save it as `.ppk`. Or if you downloaded thr `.ppk` file directly from AWS, you can skip this step.and under "Connection > SSH > Auth", browse to your `.ppk` file.
...
Windows 10 and later have a native SSH client built right into PowerShell and Command Prompt. The process is very similar to Linux/Mac, but with a few Windows-specific quirks, such as file permissions and the path to the key file. Follow along Stephan's video for the guide.

## Troubleshooting

- **The "Timeout" vs. "Refused" Distinction**:
  - **Connection Timeout**: This is 99% a **Security Group** issue. The bouncer isn't even lettng you reach the door.
...
- **The "Permision Denied" Error**: If you see `permission denied (publickey)`, it usually one of the two user errors:
