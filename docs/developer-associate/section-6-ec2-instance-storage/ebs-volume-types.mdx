...
- **gp2 (Older gen)**: Storage size and perforance are **linked**. If you want more IOPS, you are forced to pay for a bigger volume size (you get 3 IOPS per GB up to a max of 16,000 IOPS).
...
**The Database Clue**: If an exam question descibes a high-performance database cluster running on EC2 that needs consistent, ulra-low latency and maximum storage performance, skip `gp3` and look for **provisioned IOPS** (`io1` or `io2`).
