...
Stephane explicitly demonstrates the massive performance difference between bootstrapping a raw machine versus launching from a pre-baed image.
...
- **The Speed-Up**: Because Apache is already backed directly into the AMI's file system, the new instance skips the installtion download entirey. The moment the instance hits the `running` state, the web server is live immediately.
...
- **User Data (Bootstrapping)**: Slow boot times (waiting 2+ minutes for updates/packages to install). Bad for rapid scalling.
