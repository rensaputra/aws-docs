...
![](./img/codepipeline-artifact-store.png)
...
**Question 1**: An IT company leverages CodePipeline to automate its release pipelines. The development team wants to write a Lambda function that will send notifications for state changes within the pipeline.
...
- [ ] Use the CodePipeline console to set up a trigger for the Lambda function
  - **Explanation**: You cannot configure direct Lambda function triggers from within the CodePipeline console. CodePipeline emits state change events to Amazon EventBridge, where rules and targets are defined.
- [x] Set up an Amazon EventBridge rule that uses CodePipeline as an event source with the target as the Lambda function
  - **Explanation**: You can use Amazon EventBridge to detect and react to changes in the state of a pipeline, stage, or action. Then, based on rules you create, EventBridge invokes one or more target actions when a pipeline, stage, or action enters the state you specify in a rule. For the given use-case, you can set up a rule that detects pipeline changes and invokes an AWS Lambda function.  
    ![](./img/codepipeline-eventbridge.jpg)
- [ ] Set up an Amazon CloudWatch alarm that monitors status changes in Code Pipeline and triggers the Lambda function
  - **Explanation**: CloudWatch Alarms evaluate metric thresholds (such as CPU utilization or error counts) rather than matching specific event-based state transition JSON payloads. Real-time pipeline state detection requires an EventBridge rule.
- [ ] Use the Lambda console to configure a trigger that invokes the Lambda function with CodePipeline as the event source
  - **Explanation**: CodePipeline is not a direct event source supported natively inside the AWS Lambda console trigger configuration. You must route CodePipeline state changes through Amazon EventBridge.
