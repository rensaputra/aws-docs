...
- **The Strict Filename Anchor:** Ensure the text prompt box dictates the exact runtime target name: **`buildspec.yml`**.
  ![](./img/buildspec.png)
...
- **Watch the Phase Logs:** The execution phase immediately transitions from `SUBMITTED` to `COMPLETED`. However, if you look closely, the build status is marked as **`Failed`**.
