...
1. The client requests `app.domain.com.`
2. Route 53 looks at its zone file and replies: "I don't know the IP, but that text string maps to `alb-123.aws.com`."
3. The client's resolver is forced to close that session, open a completely new query, and lookup the load balancer's URL to extract the real hosting server IP address. **This extra hop adds measurable network latency**.
