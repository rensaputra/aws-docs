...
- **Step 4: Configure the Event Source Mapping (ESM)**
  ...
  - **Batch size:** Set it to `100` (This dictates the maximum number of stream records the poller will pack into a single invocation flight, bro).
...
- **Step 5: Inject Missing Stream IAM Rights (Triage) 🚨**
  ...
  - Inside the IAM console drawer, hit **Attach policies** ──► search for and link the managed **`AWSLambdaDynamoDBExecutionRole`** policy token, chief! (This instantly whitelists `GetRecords`, `GetShardIterator`, `DescribeStream`, and `ListStreams`).
