...
- [x] Use a caching strategy to write to the backend first and then invalidate the cache
  - **Explanation**: Writing to the database first and immediately invalidating (or updating) the cache key ensures that stale data is evicted. Upon the next read request, the application experiences a cache miss, fetches the fresh price/description from the database, and repopulates the cache. This pattern guarantees strong consistency while avoiding stale reads during high-traffic events.
- [ ] Use a caching strategy to write to the cache directly and sync the backend at a later time
  - **Explanation**: This write-behind/write-back approach risks data loss if the cache node fails before syncing to the database, making it unsafe for critical product pricing and transaction data.
- [ ] Use a caching strategy to update the cache and the backend at the same time
  - **Explanation**: The cache layer and database are separate distributed systems that cannot be updated simultaneously via a single atomic operation without distributed locking, which introduces extreme latency and potential failure modes.
- [ ] Use a caching strategy to write to the backend first and wait for the cache to expire via TTL
  - **Explanation**: Relying solely on TTL expiration leaves a window of time where the cache serves outdated product prices or descriptions until the TTL elapses, violating the strict requirement to keep data synchronized.
