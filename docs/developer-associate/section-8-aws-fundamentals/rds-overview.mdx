...
**Amazon Relational Database Service (RDS)** is a fully managed, relational database service that uses SQL as its query language. Instead of manually installing a database engine on an EC2 instance, patching the OS, and scripting backups, RDS automates the entire infrastructure lifecycle. Because it is a managed service, **AWS blocks all SSH access to the underlying server**, so we can only interact with the database via client applications.
...
2. **The Duration Gate**: This low storage condition must persist for **at least 5 minutes**.
