...
- **The Public Key (Share Globally)**: You upload this public file straight into the CloudFront control plane. CloudFront duplicates this key across hundreds of global Edge locations so the nodes can decrypt and verify your application's signature hashes on the fly.
...
- **Assemble the Matrix**
  ...
  - _Note_: CloudFront lets you bundle up to 5 separate public keys simultaneously inside a single key group container, allowing you to execute zero-downtime rolling key rotations with absolute ease!
