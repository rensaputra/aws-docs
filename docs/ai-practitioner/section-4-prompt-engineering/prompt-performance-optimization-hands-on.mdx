...
- [ ] A. When temperature is set to 1
- [ ] B. Temperature has no impact on hallucinations
- [ ] C. When temperature is set to 0
- [ ] D. When temperature is set to 0.5

<details>
<summary>Correct Answer</summary>

- [x] A. When temperature is set to 1
  - **Explanation:** Temperature is a parameter between 0 and 1 that regulates the creativity and randomness of the model's responses. Lower temperatures generate more deterministic, focused outputs. Setting the temperature to its maximum value (1) introduces high randomness, leading the model to select lower-probability tokens, which significantly increases the likelihood of hallucinations.  
    ![](./img/temperature-hallucination.png)

<details>
<summary>Incorrect Answers</summary>

- [ ] B. Temperature has no impact on hallucinations
  - **Explanation:** Temperature directly affects response variability and randomness; higher values make hallucinations considerably more likely.
- [ ] C. When temperature is set to 0
  - **Explanation:** Setting temperature to 0 yields greedy, fully deterministic responses focused on highest-probability tokens, which minimizes the risk of hallucinations.
- [ ] D. When temperature is set to 0.5
  - **Explanation:** A temperature of 0.5 balances determinism and variability, presenting a lower likelihood of hallucinations compared to setting it to 1.
