...
```mermaid
graph LR
    User[User Input] --> InFilter[Guardrail Input Filters]
    InFilter -->|Passes| FM[Foundation Model]
    FM --> OutFilter[Guardrail Output Filters]
    OutFilter -->|Passes| Sanitized[Sanitized Output]

    InFilter -->|Blocked Topic / Toxic / PII| Blocked1[Blocked]
    OutFilter -->|Masked PII / Blocked| Blocked2[Masked / Blocked]
```
