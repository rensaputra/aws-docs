...
[ User Query ] ------------------------+
      |                                |
      v                                v
[ Vector Search (Similarity) ] ---> [ Augmented Prompt ] ---> [ Foundation Model ] ---> [ Grounded Response + Citations ]
      ^                                (Query + Chunks)
      |
...
```
+---------------------------------------------------------------------------------------+
| INGESTION PIPELINE (Asynchronous Sync)                                                |
|                                                                                       |
|  [ Data Sources ]       [ Document Parsing ]     [ Embeddings Model ]   [ Vector DB ] |
|  * Amazon S3            * Fixed-size Chunking    * Amazon Titan         * OpenSearch  |
|  * Confluence     --->  * Semantic Chunking ---> * Cohere Embed    ---> * Aurora pgvec|
|  * SharePoint           * Metadata Tagging                              * S3 Vectors  |
|  * Webpages                                                             * Neptune     |
+---------------------------------------------------------------------------------------+
                                                                                    |
                                                                                    v
...
|  [ User Prompt ]                                                                      |
|         |                                                                             |
|         v (Embed query vector)                                                        |
|  [ Vector DB k-NN Search ] ---> Retrieves Top-k Chunks (e.g., Support Doc #1, #2)     |
|                                      |                                                |
|                                      v                                                |
|  [ Augmented Context Assembly ] = [ Original Query ] + [ Retrieved Source Chunks ]    |
|                                      |                                                |
|                                      v                                                |
|  [ Foundation Model (Bedrock) ] -----------------------------> [ Grounded Answer ]    |
|                                                                + S3 Source Citations  |
+---------------------------------------------------------------------------------------+
```
...
+--------------------------+--------------------+------------------------------------+
| Amazon OpenSearch        | Sub-10ms           | Production RAG default; high QPS,  |
| Serverless / Managed     |                    | hybrid keyword + k-NN search       |
+--------------------------+--------------------+------------------------------------+
| Amazon Aurora PostgreSQL | 10-100ms           | Hybrid SQL relational transactional|
| (with pgvector extension)|                    | data stored alongside vectors      |
+--------------------------+--------------------+------------------------------------+
| Amazon Neptune Analytics | Sub-second         | GraphRAG; relationship traversal & |
|                          |                    | multi-hop entity reasoning         |
+--------------------------+--------------------+------------------------------------+
| Amazon S3 Vectors        | Sub-100ms          | Cost-optimized cold/warm tier;     |
|                          |                    | zero idle capacity charges         |
+--------------------------+--------------------+------------------------------------+
| Third-Party Managed      | Variable           | Multi-cloud ecosystems (Pinecone,  |
| (Pinecone, Redis, Mongo) |                    | MongoDB Atlas, Redis Enterprise)   |
+--------------------------+--------------------+------------------------------------+
```
