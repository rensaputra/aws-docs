...
```mermaid
graph TD
    Settings[Bedrock Console Settings] -->|Create IAM Service Role + CloudWatch Log Group| Invocation[Bedrock Model Invocation]
    Query[Chat Playground / API Query] --> Invocation

    Invocation --> CWLogs[CloudWatch Log Stream Full Payload JSON<br/>Model ID, Region, Timestamp<br/>User Input Text & Configuration<br/>Assistant Output Message & Tokens<br/>Invocation Latency ms]
    Invocation --> CWMetrics[CloudWatch Metrics Dashboard<br/>InvocationLatency<br/>Invocations]
```
