...
```
+----------------------------------------------------------------------------------------------------+
|                               INFERENCE PURCHASING OPTIONS MATRIX                                  |
+--------------------------+-----------------------+--------------------+----------------------------+
| Inference Mode           | Billing Unit          | Commitment Terms   | Primary Architectural Fit  |
+--------------------------+-----------------------+--------------------+----------------------------+
| On-Demand                | Per Input/Output      | None               | Interactive web chatbots,  |
|                          | Token & Image Gen     | (Pay-as-you-go)    | prototyping, variable load |
+--------------------------+-----------------------+--------------------+----------------------------+
| Batch Inference          | 50% of On-Demand      | None (Job-based)   | Offline document analysis, |
|                          | Token Rates           |                    | nightly report summaries   |
+--------------------------+-----------------------+--------------------+----------------------------+
| Provisioned Throughput   | Hourly rate per Model | 1-month or 6-month | Production SLAs, high QPS, |
|                          | Unit (MU) committed   | (or no-commit 1 MU)| custom/fine-tuned models   |
+--------------------------+-----------------------+--------------------+----------------------------+
```

```
+-----------------------------------------------------------------------------------+
| BATCH INFERENCE ASYNCHRONOUS ARCHITECTURE (50% DISCOUNT)                          |
|                                                                                   |
|  [ Amazon S3 Input Bucket ] ---> Contains `input_prompts.jsonl`                   |
|              |                                                                    |
|              v (Trigger Async Batch Job)                                          |
|  [ Amazon Bedrock Batch Engine ]                                                  |
|  * Processes requests asynchronously with 50% discount                            |
|              |                                                                    |
|              v (Write Aggregated Completions)                                     |
|  [ Amazon S3 Output Bucket ] ---> Generates `output_completions.jsonl`            |
+-----------------------------------------------------------------------------------+
```
...
```
+-----------------------------------------------------------------------------------+
| MODEL ADAPTATION COST & COMPLEXITY HIERARCHY                                      |
|                                                                                   |
|  [ Domain Adaptation / Continued Pre-training ]  $$$$$ (High Compute & Unlabeled) |
|  ----------------------------------------------                                   |
|  [ Instruction-Based Supervised Fine-Tuning ]    $$$   (Moderate GPU & Labeled)   |
|  ----------------------------------------------                                   |
|  [ Retrieval-Augmented Generation (RAG) ]        $$    (Vector DB & Storage Costs)|
|  ----------------------------------------------                                   |
|  [ Prompt Engineering & System Instructions ]    $     (Zero Training Compute)    |
+-----------------------------------------------------------------------------------+
```
...
- [ ] A. The company should use serverless inference, which automatically scales resources based on traffic
- [ ] B. The company should use batch inference, thereby allowing it to run multiple inference requests in a single batch
- [ ] C. The company should use real-time inference, which is designed for low-latency responses and continuous, immediate processing
- [ ] D. The company should use on-demand inference, which allows the company to pay only for the resources consumed during each inference

<details>
<summary>Correct Answer</summary>

- [x] B. The company should use batch inference, thereby allowing it to run multiple inference requests in a single batch
  - **Explanation:** Batch inference allows you to run multiple inference requests asynchronously and improve the throughput of model inference on large datasets. In Amazon Bedrock, batch inference for supported foundation models is offered at a 50% discount compared to on-demand pricing, making it the most cost-effective option for non-latency-sensitive bulk processing.

<details>
<summary>Incorrect Answers</summary>
- [ ] A. The company should use serverless inference, which automatically scales resources based on traffic
  - **Explanation:** Serverless inference is a deployment option in Amazon SageMaker, not a native inference mode in Amazon Bedrock.
- [ ] C. The company should use real-time inference, which is designed for low-latency responses and continuous, immediate processing
  - **Explanation:** Real-time inference applies to Amazon SageMaker endpoint deployments. Furthermore, low-latency immediate processing is unnecessary and suboptimal for cost savings when delays can be tolerated.
- [ ] D. The company should use on-demand inference, which allows the company to pay only for the resources consumed during each inference
  - **Explanation:** On-demand inference is billed per token at standard rates without the 50% bulk savings provided by batch inference, making it more expensive for processing large volumes of asynchronous requests.
