...
```mermaid
flowchart LR
    A[Traditional On-Prem] --> B[You manage everything: Hardware, OS, App]
    C[IaaS] --> D[AWS manages Hardware/Virtualization; You manage OS & App]
    E[PaaS] --> F[AWS manages Hardware & OS; You manage Code & Data]
    G[SaaS] --> H[AWS manages everything; You just consume the service]
```
...
```mermaid
flowchart TD
    Main["AWS 3 PRICING FUNDAMENTALS"]

    Compute["1. COMPUTE<br/>Charged per second/hour<br/>(EC2, Lambda, Bedrock)"]
    Storage["2. STORAGE<br/>Charged per GB stored<br/>(S3, EBS, DynamoDB)"]
    Network["3. NETWORKING<br/>Pay for Data OUT only<br/>(Data IN is FREE!)"]
