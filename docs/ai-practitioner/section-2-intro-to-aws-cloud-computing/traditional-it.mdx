...
```mermaid
flowchart TD
    subgraph DataCenter["ON-PREMISES DATA CENTER<br/>Risks: Fire / Power Outage / Slow Provisioning (Weeks) / Fixed Cap"]
        direction TB
        Power["Power Grid & Backup"]
        Cooling["Cooling Systems (HVAC)"]

        Rack1["Server Rack 1"]
        Rack2["Server Rack 2"]
        RackN["Rack N"]

        Maint1["Manual HW Maintenance"]
        Maint2["Manual HW Replacement"]
        MaintN["Rack Space Depleted"]
