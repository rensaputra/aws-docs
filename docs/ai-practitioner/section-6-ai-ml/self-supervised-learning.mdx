...
- [x] D. Self-supervised learning
  - **Explanation:** Foundation models (FMs) generate their own supervisory signals directly from raw data without human annotation:
    - **Mechanism**: The model masks, hides, or shifts portions of the input data and trains itself to predict the missing pieces.
    - **Label Generation**: The "label" is derived automatically from the raw input itself (e.g., predicting the masked word in a sentence or predicting the next token in clinical notes).
    - **AWS Exam Context**: Whenever an exam item refers to foundation models learning from massive unlabelled corpora by automatically deriving labels or targets from the input sequences, AWS maps this explicitly to self-supervised learning.
